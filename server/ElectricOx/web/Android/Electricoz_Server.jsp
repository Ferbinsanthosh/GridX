                 ...
                 String longg = request.getParameter("long");
                  String HasWaitingArea = request.getParameter("HasWaitingArea");
                   String HasNearbyAmenities = request.getParameter("HasNearbyAmenities");
                    String ADDRESS = request.getParameter("ADDRESS");
                    String charging_spinnerr = request.getParameter("charging_spinnerr");
                    
                    //charging_spinnerr
                    
        String insertQry = "INSERT INTO `bunk`(`b_owner_id`,`bunk_name`,`bunk_hours`,`wifi_availability`,`restroom`,`waiting_area`,`nearBy_amenities`,`emergency_number`,`onsite_support`,`bunk_image`,`bunk_address`,`lat`,`longg` ,`Chareg_type_spinner`)VALUES('"+id+"','"+bunkName+"','"+businessHours+"','"+IsWifiAvailable+"','"+HasRestroomFacilities+"','"+HasWaitingArea+"','"+HasNearbyAmenities+"','"+emergencyContact+"','"+onSiteSupport+"','"+image+"'"
                + ",'"+ADDRESS+"','"+lat+"','"+longg+"','"+charging_spinnerr+"');";
  ...
  if (key.equals("viewEVStationsUser")) {
        ...
        if (it.hasNext()) {
            while (it.hasNext()) {
                ...
                obj.put("longg", vector.get(13).toString().trim());
                obj.put("status", vector.get(14).toString().trim());
                obj.put("ch_typ", vector.get(15).toString().trim());
